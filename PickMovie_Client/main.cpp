...
void pushMessage(const string& username, const string& password, const string& genre , const string& number) {
    try {
        context_t context(1);
        ...
        while (ventilator.handle() && !receivedMessage) {
            sleep(2000);
            string message = "PickMovie?" + username + " " + password + " " + genre + " " + number;
...
int main(int argc, char* argv[]) {
    if (argc < 4) {
        cerr << "Usage: " << argv[0] << " <username> <password> <genre>" << endl;
        return 1;
    }

    thread t(pushMessage, argv[1], argv[2], argv[3] , argv[4]);
    t.detach();
    ...
    try {
        ...
        subscriber.setsockopt(ZMQ_SUBSCRIBE, "PickMovie!", strlen("PickMovie!"));
